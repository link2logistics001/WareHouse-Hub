        ...
        if (emailExists) {
            ...
            if (!querySnapshot.empty) {
                ...
                if (existingUserType !== userType) {
                    ...
                    throw new Error(
                        `This email is already registered as ${displayLabel}. Please sign in as ${displayLabel} or use a different email.`
                    );
                }
            }
        ...
        if (userResult) {
            ...
            if (
                ...
                return {
                    ...
                    verified:
                        userData.verified ||
                        (userData.userType === 'admin' || userData.userType === 'superadmin' ? true : false),
...
export const loginWithGoogle = async (userType, isSignIn = false) => {
    ...
    try {
        ...
        // ── Admin & Data Entry override: can log in from any portal ────
        if (
            existingUserData.userType === 'superadmin' ||
            existingUserData.userType === 'admin' ||
            existingUserData.userType === 'dataentry'
        ) {
            return {
                uid: user.uid,
                email: user.email,
                ...
                verified:
                    existingUserData.verified ||
                    (existingUserData.userType === 'admin' || existingUserData.userType === 'superadmin'
                        ? true
                        : false),
        ...
        if (
            !error.code ||
            !['auth/user-not-registered', 'auth/popup-closed-by-user', 'auth/wrong-user-type'].includes(error.code)
        ) {
        }
        throw handleAuthError(error);
    } finally {

...
/**
 * AdminEditWarehouse — Admin version of AddWarehouse.
 *
 * This is almost identical to the Data Entry AddWarehouse but:
 * 1. Saves to warehouse_details/admin/emails/{email}/warehouses
 * 2. Marks source as 'admin'
 * 3. Uses a orange theme accent
 */
...
export default function AdminEditWarehouse({ setActiveTab, initialData }) {
    ...
    const handleFileChange = (photoKey, file) => {
        if (file && file.size > 10 * 1024 * 1024) {
            setErrors((prev) => ({ ...prev, [photoKey]: 'File too large — max 10 MB' }));
            return;
        }
    ...
    // ── SUBMIT — saves to admin subcollection ──
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (user.uid !== uid) {
                setSubmitError('Session mismatch.');
                setSubmitting(false);
                return;
            }
            ...
            const filesToUpload = [photos.frontView, photos.insideView, photos.dockArea, photos.rateCard].filter(
                (f) => f !== null
            );
            ...
            if (initialData) {
                ...
                docData.createdAt = initialData.createdAt || serverTimestamp();
                docData.updatedAt = serverTimestamp();

                const { doc, updateDoc } = await import('firebase/firestore');
                const docRef = initialData._docPath
                    ? doc(db, initialData._docPath)
                    : doc(db, `warehouse_details/admin/emails/${adminEmail}/warehouses`, initialData.id);
                await updateDoc(docRef, docData);
            } else {
                docData.createdAt = serverTimestamp();
                await addDoc(getWarehouseCollection('admin', adminEmail), docData);
            }
            setSubmitted(true);
        } catch (err) {
            if (err.code === 'storage/unauthorized') {
                setSubmitError('Upload blocked. Log out and retry.');
            } else if (err.code === 'storage/bucket-not-found') {
                setSubmitError('Storage not configured.');
            } else {
                setSubmitError(`Failed: ${err.message || 'Unknown error.'}`);
            }
        } finally {
            setSubmitting(false);
            setUploadProgress(0);
                ...
                >
                    <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white p-12 relative overflow-hidden">
                        ...
                        <p className="text-slate-500 mb-10 font-medium">
                            Your warehouse entry has been saved successfully.
                        </p>
                        <button
    ...
    return (
        <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 pb-24">
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-[-1]">
                ...
                <div className="absolute bottom-[20%] left-[-10%] w-[600px] h-[600px] bg-amber-500/10 rounded-full blur-[100px]" />
            </div>
                ...
                <div className="mb-10">
                    ...
                    <div className="relative mb-4 flex justify-between">
                        <div className="absolute top-1/2 left-0 w-full h-1.5 bg-slate-200/50 -translate-y-1/2 rounded-full overflow-hidden backdrop-blur-sm z-0">
                            <motion.div
                                className="h-full bg-gradient-to-r from-orange-400 to-amber-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]"
                                initial={{ width: 0 }}
                        ...
                        {Array.from({ length: totalSteps }, (_, i) => i + 1).map((s) => (
                            <div
                                key={s}
                                className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 ${s < step ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30' : s === step ? 'bg-slate-900 text-white shadow-xl ring-4 ring-slate-900/20' : 'bg-white border-2 border-slate-200 text-slate-400'}`}
                            >
                                    ...
                                    <div className="p-8 sm:p-10 flex-1">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <Field
                                                ...
                                                onChange={(v) => handleWarehouseChange('state', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            <Field
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <SelectField
                                                    ...
                                                    onChange={(v) => handleOperationsChange('daysOfOperation', v)}
                                                    mandatory
                                                    errors={errors}
                                                />
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <SelectField
                                                    ...
                                                    onChange={(v) => handlePricingChange('pricingUnit', v)}
                                                    mandatory
                                                    errors={errors}
                                                />
                                                <Field
                                                ...
                                                <div className="md:col-span-2">
                                                    <SelectField
                                                        ...
                                                        onChange={(v) => handlePricingChange('shortTermStorage', v)}
                                                        mandatory
                                                        errors={errors}
                                                    />
                                                </div>
                        ...
                        <div className="p-6 sm:px-10 border-t border-white/50 bg-white/40 backdrop-blur-md flex justify-between items-center mt-auto">
                            ...
                            {step < totalSteps ? (
                                ...
                                <button
                                    ...
                                    disabled={submitting}
                                    className="px-8 py-3.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-[0_8px_20px_rgba(249,115,22,0.3)] hover:shadow-[0_12px_25px_rgba(249,115,22,0.4)] hover:-translate-y-0.5 transition-all disabled:opacity-70 w-64 relative overflow-hidden"
                                >
                                    ...
                                    <div className="relative z-10 flex items-center gap-2">
                                        {submitting ? (
                                            ...
                                            <>
                                                <CheckCircle className="w-4 h-4 drop-shadow-sm" /> Submit Entry
                                            </>
                                        )}
                                    </div>
                                </button>
...
// ─── Shared UI Components (same as Owner but with orange accent) ───
                            ...
                            {options.map((opt) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-3 text-sm font-semibold rounded-xl transition-all flex items-center justify-between mb-0.5 ${isActive ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                                    >

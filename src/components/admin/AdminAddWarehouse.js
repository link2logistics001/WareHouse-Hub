...
import {
  ...
  AlertCircle, Loader2, Settings, DollarSign, ImageIcon, UploadCloud, X, ChevronDown, PlusCircle
} from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const docData = {
        ...
        ownerId: uid, status: 'approved', createdAt: serverTimestamp(),
        source: 'admin',
      ...
      <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 flex items-center justify-center">
        ...
        <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="max-w-lg w-full mx-auto text-center z-10 p-6">
          <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white p-12 relative overflow-hidden">
            ...
            <p className="text-slate-500 mb-10 font-medium">Your warehouse entry has been saved successfully.</p>
            <button onClick={() => setActiveTab('overview')} className="w-full px-8 py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all shadow-lg hover:-translate-y-0.5">
              Back to Dashboard
            </button>
          </div>
        </motion.div>
  ...
  return (
    <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 pb-24">
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-10 pt-10 relative z-10">
        <div className="mb-10">
          <button onClick={() => setActiveTab('overview')} className="text-slate-500 hover:text-orange-600 flex items-center gap-2 mb-6 transition-colors font-semibold bg-white/60 px-4 py-2 rounded-lg w-fit border border-white shadow-sm backdrop-blur-md">
          ...
          <div className="relative mb-4 flex justify-between">
            <div className="absolute top-1/2 left-0 w-full h-1.5 bg-slate-200/50 -translate-y-1/2 rounded-full overflow-hidden backdrop-blur-sm z-0">
              <motion.div className="h-full bg-gradient-to-r from-orange-400 to-amber-500 shadow-[0_0_10px_rgba(249,115,22,0.5)]" initial={{ width: 0 }} animate={{ width: `${((step - 1) / (totalSteps - 1)) * 100}%` }} transition={{ duration: 0.5, ease: "easeInOut" }} />
            </div>
            {Array.from({ length: totalSteps }, (_, i) => i + 1).map(s => (
              <div key={s} className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 ${s < step ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30' : s === step ? 'bg-slate-900 text-white shadow-xl ring-4 ring-slate-900/20' : 'bg-white border-2 border-slate-200 text-slate-400'}`}>
              ...
              <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="flex-1 flex flex-col">
                                ...
                                <div className="bg-white/80 p-4 rounded-2xl border border-white shadow-sm backdrop-blur-md">
                                  ...
                                  {!verifyingOtp && (
                                    <div className="mt-3 text-center">
                                      {resendCountdown > 0 ? <span className="text-[11px] font-semibold text-slate-400">Resend OTP in {resendCountdown}s</span> : <button type="button" onClick={handleSendOtp} disabled={sendingOtp} className="text-[11px] text-orange-600 hover:text-orange-700 font-bold underline transition-colors">{sendingOtp ? 'Sending...' : 'Resend OTP'}</button>}
                                    </div>
                                  )}
                                </div>
                              )}
                ...
                {step === 3 && (
                  <div className="p-8 sm:p-10 flex-1 space-y-10">
                    ...
                    <div>
                      <SectionHeading icon={<CheckCircle className="w-6 h-6 text-orange-500 drop-shadow-md" />} title="Value Added Services" subtitle="(Optional)" />
            ...
            <div className="p-6 sm:px-10 border-t border-white/50 bg-white/40 backdrop-blur-md flex justify-between items-center mt-auto">
              ...
              {step < totalSteps ? (
                ...
                <button onClick={handleSubmit} disabled={submitting} className="px-8 py-3.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-[0_8px_20px_rgba(249,115,22,0.3)] hover:shadow-[0_12px_25px_rgba(249,115,22,0.4)] hover:-translate-y-0.5 transition-all disabled:opacity-70 w-64 relative overflow-hidden">
                  {submitting && uploadProgress < 100 && <div className="absolute left-0 top-0 bottom-0 bg-orange-800/20 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />}
...
function Field({ label, id, type = 'text', placeholder, value, onChange, mandatory = false, errors = {} }) {
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={id} className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">{label} {mandatory && <span className="text-orange-500">*</span>}</label>
          ...
          <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ duration: 0.2 }} className="absolute z-50 w-full mt-2 bg-white/95 backdrop-blur-xl border border-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] overflow-hidden py-1">
            <div className="max-h-60 overflow-y-auto p-1.5">
              ...
              {options.map(opt => {
                ...
                return (
                  <button key={opt} type="button" onClick={() => { onChange(opt); setIsOpen(false); }} className={`w-full text-left px-4 py-3 text-sm font-semibold rounded-xl transition-all flex items-center justify-between mb-0.5 ${isActive ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}>
...
function YesNoField({ label, id, value, onChange, mandatory = false, errors = {} }) {
  return (
    <div className="space-y-2">
      <label className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">{label} {mandatory && <span className="text-orange-500">*</span>}</label>
      <div className="flex gap-3">
        {['Yes', 'No'].map(opt => {
          const isActive = value === opt;
          return (

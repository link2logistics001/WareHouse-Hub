...
export default function Inquiries() {
    ...
    const handleDeleteDeal = async (conv) => {
        ...
        if (
            !window.confirm(
                `Are you sure you want to remove the deal with ${conv.merchantName}? This will permanently delete the conversation and messages.`
            )
        )
            return;
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-slate-500">
                        {loading
                            ? 'Loading…'
                            : `${inquiries.length} total inquir${inquiries.length === 1 ? 'y' : 'ies'} received`}
                    </p>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full pb-8">
                    ...
                    >
                        {byStage('new').map((item) => (
                            <div
                                key={item.id}
                                className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-all"
                            >
                                ...
                                <p className="text-xs text-slate-500 mb-1 line-clamp-1">
                                    Interested in{' '}
                                    <span className="font-medium text-slate-700">{item.warehouseName}</span>
                                </p>
                                {item.spaceRequired && (
                    ...
                    >
                        {byStage('negotiating').map((item) => (
                            <div
                                key={item.id}
                                className="bg-white p-5 rounded-xl shadow-sm border-l-4 border-orange-500 hover:shadow-md transition-all"
                            >
                                ...
                                <div className="flex items-center gap-2 mb-4">
                                    <span className="px-2.5 py-1 bg-orange-100 text-orange-700 text-[10px] font-bold uppercase rounded-md tracking-wide">
                                        Negotiating
                                    </span>
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        {grantingId === item.id ? (
                                            ...
                                            <>
                                                <Unlock className="w-4 h-4" /> Grant Contact Access
                                            </>
                                        )}
                                    </button>
                    ...
                    >
                        {byStage('booked').map((item) => (
                            <div
                                key={item.id}
                                className="bg-white/80 p-4 rounded-xl border border-green-100 hover:bg-white transition-colors"
                            >
    ...
    return (
        <div className={`rounded-2xl p-4 flex flex-col h-full border ${bgColor}`}>
            ...
            <div className="space-y-3 overflow-y-auto flex-1 pr-1">
                {children}
                {items.length === 0 && <p className="text-center text-xs text-slate-300 py-8 font-medium">Empty</p>}
            </div>
        </div>
    );
}

...
export default function AddWarehouse({ setActiveTab, editingWarehouse }) {
    ...
    useEffect(() => {
        if (editingWarehouse) {
            ...
            setWarehouseDetails({
                ...
                customTypeOfConstruction: CONSTRUCTION_TYPES.includes(w.typeOfConstruction)
                    ? ''
                    : w.typeOfConstruction || '',
    ...
    const validateStep3 = () => {
        ...
        if (operationsDetails.securityFeatures.length === 0)
            e.securityFeatures = 'Select at least one security feature';
    ...
    const validateStep4 = () => {
        ...
        if (pricingDetails.pricingUnit === 'Custom' && !pricingDetails.customPricingUnit.trim())
            e.customPricingUnit = 'Please specify your custom pricing unit';
        ...
        if (pricingDetails.shortTermStorage === 'Yes' && !pricingDetails.shortTermDuration.trim())
            e.shortTermDuration = 'Please specify the short-term duration';
        return e;
    };
        ...
        try {
            ...
            const [frontViewURL, insideViewURL, dockAreaURL, rateCardURL, tariffURL] = await Promise.all([
                photos.frontView
                    ...
                    : existingPhotos.frontView,
                photos.insideView
                    ? uploadFile(photos.insideView, `${basePath}/inside_view`, (pct) =>
                          handleProgress('insideView', pct)
                      )
                    : existingPhotos.insideView,
                photos.dockArea
                    ? uploadFile(photos.dockArea, `${basePath}/dock_area`, (pct) => handleProgress('dockArea', pct))
                    : existingPhotos.dockArea,
                photos.rateCard
                    ? uploadFile(photos.rateCard, `${basePath}/rate_card`, (pct) => handleProgress('rateCard', pct))
                    : existingPhotos.rateCard,
                photos.tariff
                    ? uploadFile(photos.tariff, `${basePath}/tariff`, (pct) => handleProgress('tariff', pct))
            ...
            if (editingWarehouse) {
                // Preserve original createdAt and set updatedAt
                ...
                const docRef = editingWarehouse._docPath
                    ...
                    : doc(db, `warehouse_details/owner/emails/${ownerEmail}/warehouses`, editingWarehouse.id);
            ...
            if (err.code === 'storage/unauthorized') {
                setSubmitError(
                    'Upload blocked: auth token rejected by Storage. Please log out, log in again, and retry.'
                );
        ...
        return (
            <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 flex items-center justify-center">
                ...
                <motion.div
                    ...
                    className="max-w-lg w-full mx-auto text-center z-10 p-6"
                >
                    <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white p-12 relative overflow-hidden">
                        ...
                        <p className="text-slate-500 mb-10 font-medium">
                            Your warehouse has been saved successfully and is now entering the admin review queue.
                        </p>
                        <button
                            onClick={() => setActiveTab('dashboard')}
                            className="w-full px-8 py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all shadow-lg hover:-translate-y-0.5"
                        >
                            Back to Dashboard
                        </button>
                    </div>
        ...
        <div className="flex-1 min-h-screen relative z-0 pb-24">
            <div className="max-w-5xl mx-auto px-6 sm:px-10 pt-10 relative z-10">
                ...
                <div className="mb-10">
                    <button
                        onClick={() => setActiveTab('dashboard')}
                        className="text-slate-500 hover:text-orange-600 flex items-center gap-2 mb-6 transition-colors font-semibold bg-white/60 px-4 py-2 rounded-lg w-fit border border-white shadow-sm backdrop-blur-md"
                    >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-800 tracking-tight">
                                {editingWarehouse ? 'Edit Warehouse Details' : 'List Your Warehouse'}
                            </h1>
                            ...
                            <div
                                ...
                                className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 ${
                                    s < step
                                        ...
                                        : s === step
                                          ? 'bg-slate-900 text-white shadow-xl ring-4 ring-slate-900/20'
                    ...
                    <div className="relative z-10 flex-1 flex flex-col">
                            ...
                            >
                                ...
                                {step === 1 && (
                                    <div className="p-8 sm:p-10 flex-1">
                                        <SectionHeading
                                            icon={<User className="w-6 h-6 text-orange-500 drop-shadow-md" />}
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <SelectField
                                                ...
                                                onChange={(v) => handleOwnerChange('businessType', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            <Field
                                                label="Company Name"
                                                id="companyName"
                                                placeholder="e.g. MetroStore Pvt Ltd"
                                                value={ownerDetails.companyName}
                                                onChange={(v) => handleOwnerChange('companyName', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            <Field
                                                ...
                                                value={ownerDetails.contactPerson}
                                                onChange={(v) => handleOwnerChange('contactPerson', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            ...
                                            />
                                            <Field
                                                label={`${countryConfig.taxLabel} (Optional)`}
                                                id="ownerGstPan"
                                                placeholder="e.g. ABCDE1234F"
                                                value={ownerDetails.ownerGstPan}
                                                onChange={(v) => handleOwnerChange('ownerGstPan', v)}
                                                errors={errors}
                                            />
                                        </div>
                                    </div>
                                ...
                                {step === 2 && (
                                    <div className="p-8 sm:p-10 flex-1">
                                        <SectionHeading
                                            icon={<Building2 className="w-6 h-6 text-orange-500 drop-shadow-md" />}
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <Field
                                                ...
                                                id="warehouseName"
                                                placeholder="e.g. Prime Logistics Hub"
                                                value={warehouseDetails.warehouseName}
                                                onChange={(v) => handleWarehouseChange('warehouseName', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            <SelectField
                                                ...
                                                value={warehouseDetails.warehouseCategory}
                                                onChange={(v) => handleWarehouseChange('warehouseCategory', v)}
                                                mandatory
                                                errors={errors}
                                            />
                                            ...
                                            {warehouseDetails.warehouseCategory && (
                                                ...
                                                >
                                                    <MultiChips
                                                        ...
                                                        onToggle={(item) =>
                                                            toggleItem('storageTypes', item, setWarehouseDetails)
                                                        }
                                                        errors={errors}
                                                    />
                                                    ...
                                                    {warehouseDetails.storageTypes.includes(
                                            ...
                                            <Field
                                                ...
                                                type="number"
                                                placeholder="e.g. 30"
                                                value={warehouseDetails.clearHeight}
                                                onChange={(v) => handleWarehouseChange('clearHeight', v)}
                                                errors={errors}
                                            />
                                            <Field
                                                label="Number of Dock Doors"
                                                id="numberOfDockDoors"
                                                type="number"
                                                placeholder="e.g. 4"
                                                value={warehouseDetails.numberOfDockDoors}
                                                onChange={(v) => handleWarehouseChange('numberOfDockDoors', v)}
                                                errors={errors}
                                            />
                                            <YesNoField
                                                label="40 ft Container Handling"
                                                id="containerHandling"
                                                value={warehouseDetails.containerHandling}
                                                onChange={(v) => handleWarehouseChange('containerHandling', v)}
                                                errors={errors}
                                            />
                                            ...
                                            <Field
                                                ...
                                                placeholder="e.g. RCC, PEB, Shed, Mixed"
                                                value={warehouseDetails.typeOfConstruction}
                                                onChange={(v) => handleWarehouseChange('typeOfConstruction', v)}
                                                errors={errors}
                                            />
                                            ...
                                            <SelectField
                                                label="Warehouse Age"
                                                id="warehouseAge"
                                                options={WAREHOUSE_AGES}
                                                placeholder="Select age (optional)"
                                                value={warehouseDetails.warehouseAge}
                                                onChange={(v) => handleWarehouseChange('warehouseAge', v)}
                                                errors={errors}
                                            />
                                            <Field
                                                label={`${countryConfig.taxLabel} (Optional)`}
                                                id="warehouseGstPan"
                                                placeholder="e.g. 27AABC1234..."
                                                value={warehouseDetails.warehouseGstPan}
                                                onChange={(v) => handleWarehouseChange('warehouseGstPan', v)}
                                                errors={errors}
                                            />
                                            {/* State autocomplete */}
                                            <AutocompleteField
                                            ...
                                            <AutocompleteField
                                                ...
                                                suggestions={
                                                    CITIES_BY_STATE[warehouseDetails.state] ||
                                ...
                                {step === 3 && (
                                    <div className="p-8 sm:p-10 flex-1 space-y-10">
                                        <div>
                                            <SectionHeading
                                                icon={<Settings className="w-6 h-6 text-orange-500 drop-shadow-md" />}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <YesNoField
                                                    ...
                                                    id="inboundHandling"
                                                    value={operationsDetails.inboundHandling}
                                                    onChange={(v) => handleOperationsChange('inboundHandling', v)}
                                                    errors={errors}
                                                />
                                                <YesNoField
                                                    label="Outbound Handling"
                                                    id="outboundHandling"
                                                    value={operationsDetails.outboundHandling}
                                                    onChange={(v) => handleOperationsChange('outboundHandling', v)}
                                                    errors={errors}
                                                />
                                                <YesNoField
                                                    label="WMS Available"
                                                    id="wmsAvailable"
                                                    value={operationsDetails.wmsAvailable}
                                                    onChange={(v) => handleOperationsChange('wmsAvailable', v)}
                                                    errors={errors}
                                                />
                                                <div className="flex flex-col gap-4">
                                                    <SelectField
                                                        ...
                                                        id="daysOfOperation"
                                                        options={DAYS_OF_OPERATION}
                                                        placeholder="Select days"
                                                        value={operationsDetails.daysOfOperation}
                                                        onChange={(v) => {
                                                            handleOperationsChange('daysOfOperation', v);
                                                            if (v !== 'Other')
                                                                handleOperationsChange('customDaysOfOperation', '');
                                                        }}
                                                        mandatory
                                                        errors={errors}
                                                    />
                                                    {operationsDetails.daysOfOperation === 'Other' && (
                                                        ...
                                                        >
                                                            <Field
                                                                label="Specify Custom Days"
                                                                id="customDaysOfOperation"
                                                                placeholder="e.g. Mon, Wed, Fri only"
                                                                value={operationsDetails.customDaysOfOperation || ''}
                                                                onChange={(v) =>
                                                                    handleOperationsChange('customDaysOfOperation', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                                ...
                                                <div className="flex flex-col gap-4">
                                                    <SelectField
                                                        ...
                                                        value={operationsDetails.operationTime}
                                                        onChange={(v) => {
                                                            handleOperationsChange('operationTime', v);
                                                            if (v !== 'Other' && v !== 'Fixed Hours')
                                                                handleOperationsChange('customOperationTime', '');
                                                        }}
                                                        mandatory
                                                        errors={errors}
                                                    />
                                                    ...
                                                    {operationsDetails.operationTime === 'Fixed Hours' && (
                                                        ...
                                                        >
                                                            <Field
                                                                label="Specify Fixed Hours / Shifts"
                                                                id="customOperationTime"
                                                                placeholder="e.g. 9 AM to 6 PM or Shift 1: 6AM-2PM..."
                                                                value={operationsDetails.customOperationTime || ''}
                                                                onChange={(v) =>
                                                                    handleOperationsChange('customOperationTime', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                            ...
                                            <div className="mt-8 space-y-8">
                                                <div>
                                                    <MultiChips
                                                        ...
                                                        onToggle={(item) =>
                                                            toggleItem('securityFeatures', item, setOperationsDetails)
                                                        }
                                                        errors={errors}
                                                    />
                                                    {operationsDetails.securityFeatures.includes('Others') && (
                                                        ...
                                                        >
                                                            <Field
                                                                label="Specify Other Security Feature"
                                                                id="customSecurityFeature"
                                                                placeholder="e.g. Biometric Access"
                                                                value={operationsDetails.customSecurityFeature || ''}
                                                                onChange={(v) =>
                                                                    handleOperationsChange('customSecurityFeature', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                                ...
                                                <div>
                                                    <MultiChips
                                                        label="Suitable Goods"
                                                        id="suitableGoods"
                                                        options={SUITABLE_GOODS}
                                                        mandatory
                                                        selected={operationsDetails.suitableGoods}
                                                        onToggle={(item) =>
                                                            toggleItem('suitableGoods', item, setOperationsDetails)
                                                        }
                                                        errors={errors}
                                                    />
                                                    {operationsDetails.suitableGoods.includes('Others') && (
                                                        ...
                                                        >
                                                            <Field
                                                                label="Specify Other Goods"
                                                                id="customSuitableGood"
                                                                placeholder="e.g. Textiles, Electronics"
                                                                value={operationsDetails.customSuitableGood || ''}
                                                                onChange={(v) =>
                                                                    handleOperationsChange('customSuitableGood', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                        ...
                                        <div>
                                            <SectionHeading
                                                icon={
                                                    <CheckCircle className="w-6 h-6 text-orange-500 drop-shadow-md" />
                                                }
                                                ...
                                                {VALUE_ADDED_SERVICES.map((svc) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`px-4 py-3 rounded-2xl border text-sm font-semibold flex items-center justify-between transition-all duration-300 ${
                                                                active
                                                                    ? 'bg-orange-500 text-white border-orange-400 shadow-[0_4px_15px_rgba(249,115,22,0.3)]'
                                                                    : 'bg-white/60 backdrop-blur-sm border-white text-slate-600 hover:border-orange-200 hover:bg-white shadow-sm'
                                                            }`}
                                                        >
                                                            <span>{svc}</span>
                                                            {active && (
                                                                <CheckCircle className="w-4 h-4 text-white shrink-0 drop-shadow-sm" />
                                                            )}
                                                        </button>
                                                    );
                                                })}
                                            ...
                                            {operationsDetails.valueAddedServices.includes('Others') && (
                                                ...
                                                >
                                                    <Field
                                                        ...
                                                        id="customValueAddedService"
                                                        placeholder="e.g. Returns Processing"
                                                        value={operationsDetails.customValueAddedService || ''}
                                                        onChange={(v) =>
                                                            handleOperationsChange('customValueAddedService', v)
                                                        }
                                                        mandatory
                                                        errors={errors}
                                                    />
                                ...
                                {step === 4 && (
                                    <div className="p-8 sm:p-10 flex-1 space-y-10">
                                        <div>
                                            <SectionHeading
                                                icon={
                                                    <DollarSign className="w-6 h-6 text-emerald-500 drop-shadow-md" />
                                                }
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="flex flex-col gap-4">
                                                    <SelectField
                                                        ...
                                                        value={pricingDetails.pricingUnit}
                                                        onChange={(v) => {
                                                            handlePricingChange('pricingUnit', v);
                                                            if (v !== 'Custom')
                                                                handlePricingChange('customPricingUnit', '');
                                                        }}
                                                        mandatory
                                                        errors={errors}
                                                    />
                                                    {pricingDetails.pricingUnit === 'Custom' && (
                                                        ...
                                                        >
                                                            <Field
                                                                ...
                                                                placeholder="e.g. Per container, Per rack"
                                                                value={pricingDetails.customPricingUnit || ''}
                                                                onChange={(v) =>
                                                                    handlePricingChange('customPricingUnit', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                                ...
                                                <Field
                                                    label={`Storage Rate (${countryConfig.currency})`}
                                                    id="storageRate"
                                                    type="number"
                                                    placeholder="Approximate value allowed"
                                                    value={pricingDetails.storageRate}
                                                    onChange={(v) => handlePricingChange('storageRate', v)}
                                                    mandatory
                                                    errors={errors}
                                                />
                                                <Field
                                                    label={`Handling Fees — Optional (${countryConfig.currency})`}
                                                    id="handlingFees"
                                                    type="number"
                                                    placeholder="Leave blank if not applicable"
                                                    value={pricingDetails.handlingFees}
                                                    onChange={(v) => handlePricingChange('handlingFees', v)}
                                                    errors={errors}
                                                />
                                                <SelectField
                                                    label="Minimum Commitment Duration"
                                                    id="minCommitment"
                                                    options={MIN_COMMITMENT_OPTIONS}
                                                    placeholder="Select duration"
                                                    value={pricingDetails.minCommitment}
                                                    onChange={(v) => handlePricingChange('minCommitment', v)}
                                                    mandatory
                                                    errors={errors}
                                                />
                                                ...
                                                <div className="md:col-span-2 space-y-3">
                                                    ...
                                                    {pricingDetails.shortTermStorage === 'Yes' && (
                                                        ...
                                                        >
                                                            <Field
                                                                label="Short-Term Duration"
                                                                id="shortTermDuration"
                                                                placeholder="e.g. 1-3 Months, 3-6 Months, Flexible"
                                                                value={pricingDetails.shortTermDuration}
                                                                onChange={(v) =>
                                                                    handlePricingChange('shortTermDuration', v)
                                                                }
                                                                mandatory
                                                                errors={errors}
                                                            />
                                        ...
                                        <div>
                                            <SectionHeading
                                                icon={<ImageIcon className="w-6 h-6 text-blue-500 drop-shadow-md" />}
                                                title="Warehouse Photos & Documents"
                                            />
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                <PhotoUpload
                                                    label="Front View (Optional)"
                                                    id="frontView"
                                                    fileRef={frontViewRef}
                                                    file={photos.frontView}
                                                    onFileChange={handleFileChange}
                                                    errors={errors}
                                                />
                                                <PhotoUpload
                                                    label="Inside View (Optional)"
                                                    id="insideView"
                                                    fileRef={insideViewRef}
                                                    file={photos.insideView}
                                                    onFileChange={handleFileChange}
                                                    errors={errors}
                                                />
                                                <PhotoUpload
                                                    label="Dock Area (Optional)"
                                                    id="dockArea"
                                                    fileRef={dockAreaRef}
                                                    file={photos.dockArea}
                                                    onFileChange={handleFileChange}
                                                    errors={errors}
                                                />
                                            </div>
                                            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <PhotoUpload
                                                    label="Rate Card — PDF/JPG (Optional)"
                                                    id="rateCard"
                                                    fileRef={rateCardRef}
                                                    file={photos.rateCard}
                                                    onFileChange={handleFileChange}
                                                    errors={errors}
                                                />
                                                <PhotoUpload
                                                    label="Complete Tariff — PDF/Excel/JPG (Optional)"
                                                    id="tariff"
                                                    fileRef={tariffRef}
                                                    file={photos.tariff}
                                                    onFileChange={handleFileChange}
                                        ...
                                        {submitError && (
                                            <motion.div
                                                ...
                                                className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center gap-3 text-rose-600 backdrop-blur-sm"
                                            >
                                        ...
                                        {editingWarehouse && (
                                            ...
                                            >
                                                ...
                                                <p className="text-sm font-bold">
                                                    Note: Your warehouse will be re-verified by our team after these
                        ...
                        <div className="p-6 sm:px-10 border-t border-white/50 bg-white/40 backdrop-blur-md flex justify-between items-center mt-auto">
                            {step > 1 ? (
                                <button
                                    onClick={handleBack}
                                    className="px-6 py-3 text-slate-500 font-bold hover:text-slate-800 hover:bg-white rounded-xl transition-all border border-transparent hover:border-slate-200 shadow-sm"
                                >
                                    Back
                                </button>
                            ) : (
                            ...
                            {step < totalSteps ? (
                                <button
                                    onClick={handleNext}
                                    className="px-8 py-3.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 flex items-center gap-2 shadow-lg shadow-slate-300 hover:-translate-y-0.5 transition-all"
                                >
                                ...
                                >
                                    {submitting && uploadProgress < 100 && (
                                        <div
                                            className="absolute left-0 top-0 bottom-0 bg-orange-800/20 transition-all duration-300"
                                    ...
                                    <div className="relative z-10 flex items-center gap-2">
                                        {submitting ? (
                                            ...
                                            <>
                                                ...
                                                {editingWarehouse ? 'Update Listing' : 'Publish Listing'}
                                            </>
                                        )}
                                    </div>
                                </button>
...
function ErrMsg({ msg }) {
    return (
        <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            className="text-[11px] font-bold text-rose-500 flex items-center gap-1 mt-1.5 ml-1"
        >
    ...
    return (
        <div className="space-y-1.5 relative" ref={dropdownRef}>
            ...
            <button
                ...
                className={`w-full p-3.5 bg-white/70 backdrop-blur-sm border rounded-xl outline-none text-left transition-all flex items-center justify-between shadow-inner font-medium ${errors[id] ? 'border-rose-400 bg-rose-50/50 text-slate-800' : 'border-white text-slate-800 hover:border-orange-200/60'} ${isOpen ? 'ring-2 ring-orange-500/50 border-orange-300' : ''}`}
            >
                <span className={!value ? 'text-slate-400 font-normal' : ''}>{value || placeholder}</span>
                <ChevronDown
                    className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-orange-500' : ''}`}
                />
            </button>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute z-50 w-full mt-2 bg-white/95 backdrop-blur-xl border border-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] overflow-hidden py-1"
                    >
                        <div className="max-h-60 overflow-y-auto p-1.5 scrollbar-thin scrollbar-thumb-slate-200">
                            <button
                                type="button"
                                disabled
                                className="w-full text-left px-4 py-2 text-[11px] font-bold uppercase tracking-wider text-slate-400 cursor-default"
                            >
                                {placeholder}
                            </button>
                            {options.map((opt) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-3 text-sm font-semibold rounded-xl transition-all flex items-center justify-between mb-0.5 last:mb-0 ${isActive ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                                    >
            ...
            <div className="flex gap-3">
                {['Yes', 'No'].map((opt) => {
                    ...
                    return (
                        <label
                            key={opt}
                            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl border cursor-pointer transition-all ${isActive ? 'bg-orange-500 text-white border-orange-400 shadow-[0_4px_15px_rgba(249,115,22,0.3)]' : 'bg-white/60 backdrop-blur-sm border-white text-slate-600 hover:border-orange-200 shadow-sm'}`}
                        >
                            <input
                                type="radio"
                                name={id}
            ...
            {file ? (
                <div className="flex items-center gap-3 p-4 bg-emerald-50/80 backdrop-blur-sm border border-emerald-200 rounded-2xl shadow-sm">
                    ...
                    <button
                        ...
                        className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors shadow-sm shrink-0"
                    >
                        <X className="w-4 h-4" />
                    </button>
                </div>
                ...
                <button
                    ...
                    className={`w-full flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed rounded-2xl text-center transition-all bg-white/40 backdrop-blur-sm group ${errors[id] ? 'border-rose-400 hover:bg-rose-50/50' : 'border-slate-300 hover:border-orange-300 hover:bg-white/60'}`}
                >
                    <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform group-hover:shadow-orange-200">
                        <UploadCloud
                            className={`w-6 h-6 ${errors[id] ? 'text-rose-400' : 'text-slate-400 group-hover:text-orange-500 transition-colors'}`}
                        />
                    </div>
                    <div>

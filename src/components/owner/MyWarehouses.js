                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-500 mt-1">
                            {loading
                                ...
                                : `${warehouses.length} listing${warehouses.length !== 1 ? 's' : ''} in your network`}
                        </p>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div
                                key={i}
                                className="bg-white/40 border border-white rounded-[2rem] p-4 shadow-sm backdrop-blur-md"
                            >
                    ...
                    <motion.div
                        ...
                        className="flex flex-col items-center justify-center py-24 bg-white/60 backdrop-blur-xl border border-white rounded-[2.5rem] shadow-[0_8px_30px_-4px_rgba(0,0,0,0.04)] text-center px-4"
                    >
                        <motion.div
                            animate={{ y: [0, -10, 0] }}
                            transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
                            className="w-28 h-28 bg-gradient-to-br from-orange-50 to-orange-100 rounded-[2rem] rotate-3 flex items-center justify-center mb-6 border border-orange-200 shadow-inner"
                        >
                        ...
                        <p className="text-slate-500 max-w-sm mb-8 leading-relaxed font-medium">
                            Your portfolio is currently empty. Add your first warehouse to start tracking capacity and
                            receiving inquiries.
                        </p>
                        {setActiveTab && (
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                onClick={() => setActiveTab('add-warehouse')}
                                className="flex items-center gap-2 px-8 py-3.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-slate-300"
                            >
...
function WarehouseCard({ warehouse: w, onDelete, onEdit, variants }) {
    ...
    const handleAvailabilityChange = async (newValue) => {
        ...
        setToggling(true);
        try {
            const docRef = w._docPath
                ? doc(db, w._docPath)
                : doc(db, `warehouse_details/owner/emails/${w.email.toLowerCase().trim()}/warehouses`, w.id);
        ...
        >
            ...
            <div className="absolute top-4 left-4 z-20" ref={dropRef}>
                {isApproved ? (
                    <div className="relative">
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border backdrop-blur-xl transition-all shadow-md ${
                                isOnline
                                    ? 'bg-emerald-500/90 border-emerald-400 text-white shadow-[0_0_20px_rgba(16,185,129,0.4)]'
                        ...
                        <AnimatePresence>
                            {dropdownOpen && (
                                <motion.div
                                    ...
                                    className="absolute left-0 mt-2 w-44 bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.15)] border border-white overflow-hidden py-1 z-50"
                                >
                                    <button
                                        onClick={() => handleAvailabilityChange(true)}
                                        className={`w-full flex items-center gap-3 px-4 py-3 text-xs font-bold transition-colors ${isOnline ? 'text-emerald-600 bg-emerald-50' : 'text-slate-600 hover:bg-slate-50'}`}
                                    >
                                        <Wifi className="w-4 h-4" /> Set Online{' '}
                                        {isOnline && <CheckCircle2 className="w-4 h-4 ml-auto" />}
                                    </button>
                                    <button
                                        onClick={() => handleAvailabilityChange(false)}
                                        className={`w-full flex items-center gap-3 px-4 py-3 text-xs font-bold transition-colors ${!isOnline ? 'text-slate-700 bg-slate-100' : 'text-slate-500 hover:bg-slate-50'}`}
                                    >
                                        <WifiOff className="w-4 h-4" /> Set Offline{' '}
                                        {!isOnline && <CheckCircle2 className="w-4 h-4 ml-auto text-slate-600" />}
                                    </button>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    ...
                    <div
                        className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border shadow-sm ${badge.bg} ${badge.border} ${badge.text}`}
                    >
                        {/* Pulsing LED Dot */}
                        <div className="relative flex h-2 w-2">
                            <span
                                className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${badge.glow}`}
                            ></span>
            ...
            <div className="p-6 flex flex-col flex-1 relative rounded-b-[2rem]">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6 z-10">
                    ...
                    <StatChip
                        ...
                        label="Clear Height"
                        value={w.clearHeight ? `${w.clearHeight} ft` : '—'}
                    />
                    <StatChip
                        icon={<DoorOpen className="w-4 h-4 text-purple-500" />}
                        label="Dock Doors"
                        value={w.numberOfDockDoors ?? '—'}
                    />
                </div>

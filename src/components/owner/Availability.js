    ...
    const fetchAvailability = useCallback(async () => {
        ...
        try {
            const snap = await getDocs(
                query(collection(db, 'warehouse_availability', monthKey, 'entries'), where('owner_id', '==', user.uid))
            );
            const map = {};
            snap.docs.forEach((d) => {
                const data = d.data();
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            for (const w of warehouses) {
                ...
                if (newStatus) updated.push(w);
                else if (oldStatus) cleared.push(w);
            }
            ...
            for (const w of updated) {
                const payload = {
                    ...
                    owner_type: user.userType || 'warehouse_partner',
                    date: modalDate,
                    month: saveMonth,
                    status: modalStatuses[w.id],
                    warehouse_name: w.warehouseName || w.name || 'Warehouse',
                    updated_at: new Date().toISOString(),
                };
                await setDoc(
                    doc(db, 'warehouse_availability', saveMonth, 'entries', availDocId(w.id, modalDate)),
                    payload,
                    { merge: true }
                );
            }
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-500 mt-1">
                            Manage schedules and blocks for your properties in real-time.
                        </p>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    >
                        ...
                        <p className="text-slate-500 max-w-sm font-medium">
                            Add a warehouse first to start managing its availability calendar.
                        </p>
                    </motion.div>
                ) : (
                    <div>
                        ...
                        `}
                                                ...
                                                {!hasStatuses && (
                                                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <div className="w-7 h-7 bg-white rounded-full shadow-sm flex items-center justify-center text-orange-500 border border-orange-100">
                                            ...
                                            <div className="mt-4 space-y-1.5 relative z-10">
                                                {statuses.slice(0, 3).map((s) => (
                                                    <div
                                                        key={s.warehouseId}
                                                        className="group/strip relative flex items-center w-full h-6 bg-slate-100 rounded-md overflow-hidden border border-slate-200/50"
                                                    >
                                                        {/* Glowing LED Bar */}
                                                ...
                                                {statuses.length > 3 && (
                                                    <p className="text-[10px] text-slate-400 font-bold pl-1 mt-2">
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="p-6 sm:p-8 space-y-4 overflow-y-auto custom-scrollbar bg-slate-50/50 flex-1 relative z-10">
                                {warehouses.length === 0 && (
                                    <p className="text-slate-400 text-sm text-center font-bold">
                                ...
                                {warehouses.map((w) => {
                                    ...
                                    const statusInfo = hasStatus
                                        ? STATUS_MAP[currentStatus] || STATUS_MAP.available
                                        : null;

                                    return (
                                        <div
                                            key={w.id}
                                            className={`p-4 rounded-3xl border-2 transition-all duration-300 bg-white/80 backdrop-blur-md ${hasStatus ? `${statusInfo.border} shadow-[0_4px_20px_rgba(0,0,0,0.03)]` : 'border-white shadow-sm hover:border-orange-200'}`}
                                        >
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <div
                                                        className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner ${hasStatus ? statusInfo.bgLight + ' ' + statusInfo.text : 'bg-slate-100 text-slate-400'}`}
                                                    >
                                                        {(w.warehouseName || w.name || 'W').charAt(0).toUpperCase()}
                                                    </div>
                                                    <div className="min-w-0">
                                                        <p className="font-bold text-slate-800 text-base truncate mb-0.5">
                                                            {w.warehouseName || w.name || 'Unnamed Property'}
                                                        </p>
                                                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 truncate flex items-center gap-1.5">
                                                            <MapPin className="w-3 h-3 text-orange-400" />{' '}
                                                            {w.city || 'Location Pending'}
                                                        </p>
                                                    </div>
                                                </div>
                            ...
                            `}
                                                    ...
                                                    <ChevronDown
                                                        className={`absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${hasStatus ? statusInfo.text : 'text-slate-400'}`}
                                                    />
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="flex items-center justify-end gap-3 p-6 sm:p-8 border-t border-white/50 bg-white/60 backdrop-blur-md relative z-10">
                                <button
                                    onClick={closeModal}
                                    disabled={saving}
                                    className="px-6 py-3.5 text-sm font-bold text-slate-500 hover:text-slate-800 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all disabled:opacity-50 shadow-sm hover:shadow-md"
                                >
                                    Cancel
                                </button>
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.97 }}
                                    className="px-8 py-3.5 text-sm font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl hover:shadow-[0_8px_25px_rgba(249,115,22,0.4)] border border-orange-400/50 transition-all disabled:opacity-70 flex items-center gap-2 shadow-lg shadow-orange-500/20"
                                >
                                    {saving ? (
                                        <>
                                        ...
                                        <>
                                            <Save className="w-4 h-4" /> Confirm Schedule
                                        </>
                                    )}
                                </motion.button>
                            </div>
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        className={`fixed bottom-8 right-8 z-[60] px-6 py-4 rounded-2xl shadow-2xl border flex items-center gap-3 text-sm font-bold backdrop-blur-xl ${toast.type === 'success' ? 'bg-emerald-500/90 text-white border-emerald-400 shadow-[0_10px_30px_rgba(16,185,129,0.3)]' : 'bg-rose-500/90 text-white border-rose-400 shadow-[0_10px_30px_rgba(244,63,94,0.3)]'}`}
                    >
                        {toast.type === 'success' ? (

...
import { Building2, MapPin, ChevronDown, Activity, Plus, Calendar, ArrowUpRight, Sparkles } from 'lucide-react';
        ...
        const fetchMyWarehouses = async () => {
            ...
            try {
                ...
                fetchedWarehouses.forEach((data) => {
                    ...
                    if (data.status === 'Live' || data.status === 'Approved' || data.status === 'approved')
                        calcActive += 1;
                });
        ...
        <div className="flex-1 min-h-screen relative z-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            Here is the latest telemetry for your logistics portfolio.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2.5 bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-xl text-sm font-semibold text-slate-700 hover:border-orange-300 hover:shadow-md hover:shadow-orange-500/10 transition-all focus:ring-2 focus:ring-orange-500/20 outline-none"
                        >
                            <Calendar size={16} className="text-slate-400" />
                            {selectedTimeframe}
                            <motion.div animate={{ rotate: isTimeframeOpen ? 180 : 0 }}>
                        ...
                        <AnimatePresence>
                            {isTimeframeOpen && (
                                <motion.div
                                    ...
                                    className="absolute right-0 mt-3 w-40 bg-white/90 backdrop-blur-xl border border-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] overflow-hidden z-50 p-1"
                                >
                                    {timeframes.map((time) => (
                                        <button
                                            key={time}
                                            ...
                                            className={`w-full text-left px-4 py-2.5 text-sm font-medium rounded-xl transition-colors ${selectedTimeframe === time ? 'text-orange-600 bg-orange-50' : 'text-slate-600 hover:bg-slate-50'}`}
                                        >
                                            {time}
                                        </button>
                                    ))}
                    ...
                    <motion.button
                        whileHover={{ scale: 1.03, boxShadow: '0px 10px 25px rgba(249, 115, 22, 0.4)' }}
                        whileTap={{ scale: 0.97 }}
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="p-10 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    {[
                        ...
                        },
                        {
                            ...
                            path: 'M5 10 Q 30 5, 50 20 T 95 15',
                            isDec: false,
                        },
                    ].map((card, i) => (
                        <motion.div
                            ...
                            className="bg-white/80 p-6 border border-white hover:border-slate-200 rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow relative overflow-hidden group"
                        >
                            <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 group-hover:text-slate-600 transition-colors">
                                {card.title}
                            </h3>
                            <div className="flex justify-between items-end">
                                <div>
                                    <span className="text-4xl font-black text-slate-800 tracking-tight drop-shadow-sm">
                                        {loading ? (
                                            <SkeletonPulse className="w-12 h-10 inline-block align-bottom" />
                                        ) : (
                                            <AnimatedNumber value={card.value} isDecimal={card.isDec} />
                                        )}
                                    </span>
                                    {!loading && card.suffix && (
                                        <span className="text-sm font-bold text-slate-400 ml-1">{card.suffix}</span>
                                    )}
                                </div>
                                <AnimatedTrendLine color={card.color} glowColor={card.glow} pathData={card.path} />
                            </div>
                            <div
                                className={`absolute inset-0 bg-gradient-to-tr from-${card.color.split('-')[1]}-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`}
                            />
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <motion.div
                        variants={cardVariants}
                        className="lg:col-span-2 bg-white/80 border border-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] p-8 relative overflow-hidden"
                    >
                        <div className="flex justify-between items-center mb-8 relative z-10">
                            ...
                            <button
                                ...
                                className="text-xs font-bold text-orange-600 hover:text-white bg-orange-50 hover:bg-orange-500 hover:shadow-lg hover:shadow-orange-500/20 px-5 py-2.5 rounded-xl transition-all"
                            >
                                View All Directory
                            </button>
                        </div>
                        ...
                        <div className="space-y-4 relative z-10">
                            {loading ? (
                                ...
                                [1, 2, 3].map((i) => (
                                    <div
                                        key={i}
                                        className="flex items-center justify-between p-4 rounded-2xl border border-white bg-white/40"
                                    >
                                ...
                                <motion.div
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="py-16 text-center border-2 border-dashed border-slate-200/60 rounded-3xl bg-white/30 backdrop-blur-sm"
                                >
                                        ...
                                        <div className="flex items-center gap-5">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 mt-1">
                                                    ...
                                                    {wh.city || 'Location pending'}, {wh.state || ''}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-8">
                                            <div className="text-right hidden sm:block">
                                                ...
                                                <span className="text-sm font-bold text-slate-700">
                                                    {Number(wh.totalArea || 0).toLocaleString()} sq ft
                                                </span>
                                            </div>
                                            <span
                                                className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-full border ${wh.status === 'Live' ? 'bg-emerald-50 text-emerald-600 border-emerald-200/50 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-orange-50 text-orange-600 border-orange-200/50 shadow-[0_0_15px_rgba(249,115,22,0.15)]'}`}
                                            >
                                                {wh.status || 'Draft'}
                                            </span>
                                            <button
                                                onClick={() => setActiveTab('my-warehouses')}
                                                className="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-all opacity-0 group-hover:opacity-100"
                                            >
                    ...
                    <motion.div
                        variants={cardVariants}
                        className="bg-white/80 border border-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] p-8 flex flex-col relative overflow-hidden group hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow duration-500"
                    >
                        ...
                        {(() => {
                            ...
                            const dataPoints = sorted.map((w) => {
                                ...
                                return {
                                    time: ts,
                                    area: cumulative,
                                    name: w.companyName || w.warehouseName || 'Warehouse',
                                };
                            });
                                    ...
                                    <div className="flex-1 flex items-center justify-center h-48">
                                        <div className="text-center">
                                            ...
                                            <p className="text-xs text-slate-300 mt-1">
                                                Add warehouses to see your growth chart
                                            </p>
                                        </div>
                                    </div>
                                );
                            ...
                            const points = dataPoints.map((d, i) => ({
                                x:
                                    dataPoints.length === 1
                                        ? svgW / 2
                                        : (i / (dataPoints.length - 1)) * (svgW - 80) + 40,
                            ...
                            const growthPct =
                                firstArea > 0 ? (((lastArea - firstArea) / firstArea) * 100).toFixed(1) : '—';
                            ...
                            const timeLabels =
                                dataPoints.length <= 4
                                    ...
                                    : [
                                          new Date(dataPoints[0].time).toLocaleDateString('en-IN', {
                                              day: 'numeric',
                                              month: 'short',
                                          }),
                                          new Date(
                                          ...
                                          new Date(dataPoints[dataPoints.length - 1].time).toLocaleDateString('en-IN', {
                                              day: 'numeric',
                                              month: 'short',
                                          }),
                                      ];
                                ...
                                <>
                                        ...
                                        <div className="absolute inset-0 flex justify-between px-6 pt-4 pb-6 pointer-events-none">
                                            {[1, 2, 3, 4].map((i, index) => (
                                                <motion.div
                                                    ...
                                                    className="w-px border-l border-dashed border-slate-200/60"
                                                ></motion.div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="mt-6 pt-4 border-t border-slate-100/50 flex justify-between items-center bg-white/50 border border-white shadow-sm rounded-2xl p-4 relative z-10 backdrop-blur-md">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <span className="font-bold text-emerald-500 text-sm drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]">
                                                {(lastArea / 1000).toFixed(1)}k sq ft
                                            </span>
                                        </div>
                                    </div>
                                </>

...
export default function MerchantQuotations({ user }) {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            case 'Viewed':
                return (
                    <span className="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">Viewed</span>
                );
            case 'Sent':
            default:
                return (
                    <span className="px-3 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded-full">New</span>
                );
        }
    };
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-sm font-medium text-slate-500 mt-1">
                        Review and manage price quotes received from warehouse partners.
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
                    ...
                    <p className="text-slate-500 text-sm mb-6 max-w-sm mx-auto">
                        When you request quotes from warehouse partners, they will appear here for your review.
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {quotations.map((quote) => (
                        <div
                            key={quote.id}
                            className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all relative overflow-hidden flex flex-col justify-between"
                        >
                            <div className="flex items-start justify-between mb-4">
                                <div>
                                    <h3 className="font-bold text-slate-900 text-lg mb-1">
                                        {quote.party_details?.provider_name || 'Warehouse Partner'}
                                    </h3>
                            ...
                            <div className="text-sm text-slate-600 mb-6 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                <p>
                                    <strong className="text-slate-800">Date Received:</strong>{' '}
                                    {quote.sent_at
                                        ? new Date(quote.sent_at.seconds * 1000).toLocaleDateString()
                                        : 'Unknown'}
                                </p>
                                {quote.valid_until && (
                                    <p>
                                        <strong className="text-slate-800">Valid Until:</strong> {quote.valid_until}
                                    </p>
                                )}
                            </div>

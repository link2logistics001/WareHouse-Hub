...
export default function DEInquiries() {
    ...
    const handleGrantContact = async (inq) => {
        ...
        try {
            await grantContactAccess(inq.id);
        } catch (err) {
            console.error(err);
        } finally {
            setGrantingId(null);
    ...
    const handleDeleteConversation = async (inq) => {
        ...
        try {
            await deleteConversation(inq.id);
        } catch (err) {
            console.error(err);
        ...
        <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0">
            ...
            <div className="bg-white/90 backdrop-blur-sm border-b border-white sticky top-0 z-20 px-10 py-6">
                ...
                <p className="text-sm text-slate-500 mt-1">
                    {inquiries.length} total conversation{inquiries.length !== 1 ? 's' : ''}
                </p>
            </div>
                            ...
                            return (
                                <div key={stage}>
                                    <div className="flex items-center gap-3 mb-4">
                                        <span
                                            className={`px-3 py-1 rounded-full text-xs font-bold border ${cfg.color} flex items-center gap-1.5`}
                                        >
                                            {cfg.icon} {cfg.label}
                                        </span>
                                    ...
                                    <div className="grid gap-4">
                                        {items.map((inq) => (
                                            <div
                                                key={inq.id}
                                                className="bg-white/80 rounded-2xl border border-white shadow-sm hover:shadow-md transition-all p-5"
                                            >
                                                <div className="flex justify-between items-start">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <p className="text-sm text-slate-500 mt-0.5">
                                                            {inq.merchantName} · {inq.merchantEmail}
                                                        </p>
                                                        {inq.spaceRequired && (
                                                            <p className="text-xs text-slate-400 mt-1">
                                                                Space: {inq.spaceRequired}
                                                            </p>
                                                        )}
                                                        {inq.message && (
                                                    ...
                                                    <div className="flex items-center gap-2 shrink-0 ml-4">
                                                        <button
                                                            ...
                                                            className="px-4 py-2 bg-cyan-50 text-cyan-700 hover:bg-cyan-100 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 border border-cyan-200"
                                                        >
                                                            <MessageSquare className="w-3 h-3" /> Chat
                                                        </button>
                                                        {!inq.contactAccessGranted && (
                                                            <button
                                                                onClick={() => handleGrantContact(inq)}
                                                                disabled={grantingId === inq.id}
                                                                className="px-3 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-xl text-xs font-bold transition-all border border-emerald-200 flex items-center gap-1"
                                                            >
                                                                {grantingId === inq.id ? (
                                                        ...
                                                        {cfg.next && (
                                                            <button
                                                                onClick={() => moveToStage(inq.id, cfg.next)}
                                                                disabled={movingId === inq.id}
                                                                className="px-3 py-2 bg-slate-900 text-white hover:bg-slate-800 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5"
                                                            >
                                                                {movingId === inq.id ? (
                                                                    <Loader2 className="w-3 h-3 animate-spin" />
                                                                ) : (
                                                        ...
                                                        <button
                                                            onClick={() => handleDeleteConversation(inq)}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm pointer-events-none">
                    <div className="bg-white rounded-3xl p-10 text-center shadow-2xl max-w-sm">
                        ...
                        <p className="text-sm text-slate-500">
                            {celebrationDetails.warehouseName} has been booked by {celebrationDetails.merchantName}.
                        </p>
                    </div>
                </div>
            )}

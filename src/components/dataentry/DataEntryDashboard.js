    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setUser((prev) => ({
                ...
                nameChanged: updated.nameChanged,
            }));
            setEditing(false);
        ...
        <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 pb-20">
            ...
            <div className="max-w-2xl mx-auto p-10 space-y-8">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="relative group">
                            ...
                            <button
                                ...
                                disabled={uploading}
                                className="absolute -bottom-2 -right-2 w-8 h-8 bg-slate-900 text-white rounded-full flex items-center justify-center shadow-md hover:bg-slate-800 transition-all"
                            >
                                {uploading ? (
                ...
                <div className="bg-white/80 backdrop-blur-md rounded-3xl border border-white shadow-sm p-8">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        {!editing && (
                            <button
                                ...
                                className="text-xs text-cyan-600 font-bold flex items-center gap-1 hover:text-cyan-700 transition-colors"
                            >
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            {editing ? (
                                <input
                                    ...
                                    className="w-full p-3 bg-white/70 border border-white rounded-xl focus:ring-2 focus:ring-cyan-500/50 outline-none text-slate-800 font-medium disabled:opacity-50"
                                />
                            ) : (
                                <p className="text-sm font-semibold text-slate-800">{user?.name || '—'}</p>
                            )}
                            {user?.nameChanged && editing && (
                                <p className="text-[10px] text-amber-600 mt-1">Name already changed once</p>
                            )}
                        </div>
                        <div>
                            <label className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 block">
                            ...
                            {editing ? (
                                <input
                                    ...
                                    className="w-full p-3 bg-white/70 border border-white rounded-xl focus:ring-2 focus:ring-cyan-500/50 outline-none text-slate-800 font-medium"
                                />
                            ) : (
                        ...
                        <div>
                            ...
                            <p className="text-sm font-semibold text-cyan-600 flex items-center gap-2">
                                <Shield className="w-3 h-3" /> Data Entry
                            </p>
                        </div>
                    </div>
                    {editing && (
                        <div className="mt-6 flex items-center gap-3">
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-50"
                            >
                    ...
                    {msg && (
                        <p
                            className={`mt-4 text-sm font-semibold flex items-center gap-1.5 ${msg.includes('saved') ? 'text-emerald-600' : 'text-red-500'}`}
                        >
                            {msg.includes('saved') ? (
            ...
            <main className="flex-1 transition-all duration-300">
                ...
                <header className="md:hidden bg-white/90 backdrop-blur-sm border-b border-white px-4 py-3 flex items-center justify-between sticky top-0 z-20">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 bg-cyan-600 rounded-md flex items-center justify-center text-white font-bold text-xs">
                            DE
                        </div>

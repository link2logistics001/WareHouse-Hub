/**
 * DECalendar.js — Data Entry Capacity & Booking Manager
 *
 * Data Entry variant of the Owner's Availability.js dashboard component.
 * Functionally identical but fetches warehouses from the 'dataentry' role
 * path instead of 'warehouse_partner'.
 *
 * Organizes capacity bookings directly in the warehouse document
...
export default function DECalendar({ onOpenSidebar }) {
    const { user } = useAuth();
    ...
    // Core warehouse fetcher (configured for 'dataentry' role)
    ...
    return (
        <div className="flex-1 bg-[#f4f5f7] min-h-screen relative overflow-hidden z-0 pb-20">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center px-10 py-8 bg-white/90 backdrop-blur-sm border-b border-white sticky top-0 z-30 shadow-[0_4px_30px_rgba(0,0,0,0.02)] gap-4">
                <div>

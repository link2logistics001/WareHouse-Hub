                ...
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            {warehouses.length} warehouse{warehouses.length !== 1 ? 's' : ''} submitted
                        </p>
                    </div>
                    <motion.button
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="p-10 relative z-10">
                {warehouses.length === 0 ? (
                    <motion.div variants={cardVariants} className="py-20 text-center">
                        ...
                        <button
                            onClick={() => setActiveTab('add-warehouse')}
                            className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold shadow-md hover:bg-slate-800 transition-all"
                        >
...
function DEWarehouseCard({ w, onDelete, onEdit }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const docRef = w._docPath
                ? doc(db, w._docPath)
                : doc(db, `warehouse_details/dataentry/emails/${user.email.toLowerCase().trim()}/warehouses`, w.id);
    ...
    return (
        <motion.div
            layout
            className="bg-white/80 rounded-3xl border border-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-all overflow-hidden"
        >
            <div className="p-6 flex items-center justify-between">
                    ...
                    <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                            ...
                            {w.measurementUnit === 'mt' ? (
                                ...
                                <span className="flex items-center gap-1">
                                    <Layers className="w-3 h-3" />
                                    {Number(w.totalArea || 0).toLocaleString()} {config.unit} |{' '}
                                    {Number(w.totalMetricTons || 0).toLocaleString()} MT
                                </span>
                            ) : (
                                w.totalArea && (
                                    <span className="flex items-center gap-1">
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <span
                        className={`px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-full border flex items-center gap-1.5 ${status.className}`}
                    >
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
                                {[
                                    ...
                                    { label: 'Hours', value: w.operationTime || '—' },
                                    {
                                        label: 'Pricing',
                                        value: w.storageRate
                                            ? `${fmtPrice(w.storageRate)} / ${w.pricingUnit || config.unit}`
                                            : '—',
                                    },

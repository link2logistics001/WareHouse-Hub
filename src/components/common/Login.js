...
const WH_ALL_SLOTS = [];
for (let r = 0; r < 3; r++)
    for (let lv = 0; lv < 3; lv++) for (let col = 0; col < 3; col++) WH_ALL_SLOTS.push({ rackIdx: r, col, level: lv });
...
function WhScanBeam({ slot }) {
    ...
    return (
        <line
            ...
            strokeWidth="1.5"
            strokeLinecap="round"
            style={{ animation: 'wh_scanFade 0.8s ease both' }}
        />
    );
}
      ...
      `}</style>
            ...
            <div style={{ animation: 'wh_fadeUp 0.6s ease 0.1s both', zIndex: 5, paddingTop: '8px' }}>
                ...
                >
                    Join the
                    <br />
                    <span style={{ color: WH_ORANGE }}>Hub.</span>
                </h2>
                <div
                    style={{
                        width: '36px',
                        height: '2px',
                        background: WH_ORANGE,
                        marginTop: '16px',
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', zIndex: 4, margin: '0 -8px' }}>
                ...
                >
                    ...
                    {beams.map((b) => (
                        <WhScanBeam key={b.id} slot={b.slot} />
                    ))}
...
function RoleStep({ onSelect }) {
    const roles = [
        {
            ...
            icon: (
                <svg
                    className="w-10 h-10 text-slate-400 group-hover:text-[#E65100] transition-colors duration-300"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    strokeWidth={1.4}
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[420px] w-full relative">
            ...
            <div
                className="absolute top-4 left-8 w-16 h-16 rounded-full border border-orange-200/40 opacity-70 animate-pulse hidden sm:block"
                style={{ animationDuration: '4s' }}
            ></div>
            <div
                className="absolute bottom-12 right-6 w-24 h-24 rounded-full border border-orange-100/50 opacity-60 hidden sm:block"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full max-w-lg mx-auto relative z-10">
                {roles.map((role) => (
                    ...
                    >
                        ...
                        <span className="font-bold text-slate-900 text-lg relative z-10 group-hover:text-[#E65100] transition-colors duration-300">
                            {role.label}
                        </span>
    ...
    if (showVerificationSent) {
        return (
            <div className="text-center py-8">
                ...
                {error && (
                    <p
                        className={`mb-6 text-sm font-bold ${error.includes('sent') ? 'text-emerald-600' : 'text-rose-500'}`}
                    >
                        {error}
                    </p>
                )}
            ...
            <div className="w-full">
                <div className="flex items-center gap-3 mb-8">
                    <button
                        ...
                        className="p-2 -ml-2 rounded-full text-slate-400 hover:text-[#E65100] transition-colors"
                    >
                ...
                <form onSubmit={handleResetPassword} className="space-y-5">
                    ...
                    {error && (
                        <p
                            className={`text-sm font-bold ${error.includes('sent') ? 'text-emerald-600' : 'text-rose-500'}`}
                        >
                            {error}
                        </p>
                    )}
    ...
    return (
        <div className="w-full">
            <div className="flex items-center justify-between mb-8">
                <span
                    className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${roleColor}`}
                >
            ...
            <div className="flex gap-2 mb-8 p-1.5 bg-slate-100 rounded-2xl">
                <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-xl font-bold transition-all ${isLogin ? 'bg-white text-[#E65100] shadow-sm' : 'text-slate-500'}`}
                >
                    Sign In
                </button>
                <button
